...
static jobject fridaDetectListenerObj = NULL;
static JavaVM* jvm = NULL;

static jmethodID fridaDetectListenerMethodID = NULL;
...
// Helper function to call callback from any thread
__attribute__((always_inline))
void triggerCallback() {
    if (jvm == NULL || fridaDetectListenerObj == NULL || fridaDetectListenerMethodID == NULL) {
        return;
    }
    JNIEnv *env;
    ...
    (*env)->CallVoidMethod(env, fridaDetectListenerObj, fridaDetectListenerMethodID);
...
Java_co_pango_detectfrida_DetectFridaKt_initializeNative(JNIEnv *env, jclass clazz,
                                                         jobject frida_detected_listener) {
    // Clean up previous callback if exists
    if (fridaDetectListenerObj != NULL) {
        (*env)->DeleteGlobalRef(env, fridaDetectListenerObj);
    }

    // Create a global reference to prevent garbage collection
    fridaDetectListenerObj = (*env)->NewGlobalRef(env, frida_detected_listener);

    // Cache the VM for later use in other threads
    (*env)->GetJavaVM(env, &jvm);

    // Get the method ID (cache this)
    jclass callbackClass = (*env)->GetObjectClass(env, frida_detected_listener);
    fridaDetectListenerMethodID = (*env)->GetMethodID(env, callbackClass,
                                             "onDetected", "()V");
    detectfrida();
}
